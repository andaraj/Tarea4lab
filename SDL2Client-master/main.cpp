...
int main(int argc, char **argv)
{
    ...
    while (shutdownClient == false)
    {
        ...
        if (socketActive != 0)
        {
            ...
            if (messageFromServer != 0)
            {
                ...
                if(strcmp(buffer,"exit")==0)
                    exit(0);
                if(strcmp(buffer,"circulo")==0)
                {
                    background = IMG_LoadTexture(renderer,"circulo.png");

                }
                if(strcmp(buffer,"cuadrado")==0)
                {
                background = IMG_LoadTexture(renderer,"cuadrado.png");
                SDL_RenderPresent(renderer);
                }

                if(strcmp(buffer,"triangulo")==0)
                {
                background = IMG_LoadTexture(renderer,"triangulo.png");
        ...
        //SDL_RenderCopy(renderer, circulo, NULL, &rect_circulo);
        SDL_RenderPresent(renderer);
